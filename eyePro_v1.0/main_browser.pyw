...
def pause(e):
    try:
        with open('c:\eyePro\log.txt','a') as log: #here_path+
            log.write('Pause at:'+str(time.asctime())+'\n')     
        logger('pause ',do= 2, path='c:/eyePro/log')
        os.system(e)
        keyboard.remove_hotkey(pau)
    except:
        try:
            keyboard.remove_hotkey(pau)
        except:
...
def target_job0():
    ...
    chgBg(line)
    try:
        os.popen('"C:\Program Files (x86)\Google\Chrome\Application\chrome.exe"  --kiosk "C:\\eyePro\\relax_web\\Interface\\interface.html"')
    except:
        os.popen('"C:\Program Files (x86)\Internet Explorer\iexplore.exe" --k "C:\\eyePro\\relax_web\\Interface\\interface.html"')
    os.popen('"C:\Program Files (x86)\Windows Media Player\wmplayer.exe" '+file)
